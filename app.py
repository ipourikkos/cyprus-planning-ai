...
def health() -> dict[str, Any]:
    ...
    return {
        ...
        "market_source_registry": {
            "source_count": source_status_summary().get("source_count"),
            "runnable_count": source_status_summary().get("runnable_count"),
        },
...
const map=L.map("map",{zoomControl:false}).setView([35.05,33.2],9);L.control.zoom({position:"bottomleft"}).addTo(map);L.esri.tiledMapLayer({url:"https://eservices.dls.moi.gov.cy/arcgis/rest/services/National/CadastralMap_EN/MapServer"}).addTo(map);
...
async function analyse(requestId,parcelId){const geom=currentSite.geometry_metrics||{};const planningPromise=post('/api/parcel-planning-analysis',{parcel_id:parcelId}).catch(()=>null);const oppPromise=post('/api/parcel-opportunity-analysis',{parcel_id:parcelId,assumptions:{},centroid_lat:geom.centroid_lat??null,centroid_lon:geom.centroid_lon??null}).catch(()=>null);const extrasPromise=post('/api/site-extra',{parcel_feature:currentSite.parcel_feature}).catch(()=>null);[currentPlanning,currentOpportunity,extras]=await Promise.all([planningPromise,oppPromise,extrasPromise]);if(requestId!==selectionRequestId)return;if(extras)currentSite={...currentSite,...extras};currentProposals=await post('/api/parcel-proposals',{parcel_id:parcelId,planning_analysis:currentPlanning,opportunity_analysis:currentOpportunity});if(requestId!==selectionRequestId)return;$("confidence").textContent=currentPlanning?.confidence?`${currentPlanning.confidence} confidence`:"Preliminary";renderOptions();renderValue();renderIssues();renderThought();}
async function selectSite(lat,lon){const requestId=++selectionRequestId;currentPlanning=currentOpportunity=currentProposals=null;activeOption=0;try{const br=await fetch(`/api/parcel-basic?lat=${encodeURIComponent(lat)}&lon=${encodeURIComponent(lon)}`),basic=await br.json();if(!br.ok)throw new Error(basic.detail||'Parcel lookup failed');if(requestId!==selectionRequestId)return;if(parcelLayer)map.removeLayer(parcelLayer);parcelLayer=L.geoJSON(basic.parcel_feature,{style:{color:'#173f2b',weight:3,fillOpacity:.16}}).addTo(map);map.fitBounds(parcelLayer.getBounds(),{padding:[35,35]});const dr=await fetch(`/api/parcel-details?parcel_id=${encodeURIComponent(basic.parcel_id)}`),details=await dr.json();if(!dr.ok)throw new Error(details.detail||'Parcel details failed');if(requestId!==selectionRequestId)return;currentSite={...details,parcel_feature:basic.parcel_feature,geometry_metrics:basic.geometry_metrics||{}};showParcel();$("optionBody").innerHTML='<div class="loading">Calculating viable development options…</div>';$("valueBody").innerHTML='<div class="loading">Checking market evidence…</div>';analyse(requestId,basic.parcel_id);}catch(e){alert(e.message||e)}}
