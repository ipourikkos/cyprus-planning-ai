...
APP_BUILD = "2026-07-18-market-cost-value-fix-v5"
...
RESEARCH PRIORITY
1. Exact road/quarter/locality and immediately adjacent listings or developments.
2. Same municipality micro-market.
3. District only when local evidence is genuinely thin.

VALUATION RULES
- Prices must exclude VAT so PLANA does not mix VAT-inclusive and VAT-exclusive figures.
- Land figures must reflect current public ASKING evidence for comparable bare development land; exclude sites sold with completed buildings, permits, major sea-front premiums, or clearly superior planning rights unless the subject shares those characteristics.
...
- Critically, estimate the total achievable sale range for ONE newly completed detached house developed on a plot comparable to this subject site. Include the location/land/view/pool/landscaping premium in that total value. Do not discard adjacent €10m+ luxury homes as outliers when the subject is in the same prime micro-market.
- Generic construction €/m² is for ordinary apartment/office/shop hard construction.
- Detached-house construction €/m² is for the relevant market tier and includes high-spec finishes and MEP on TOTAL constructed area, but excludes land, VAT, professional/finance/sales costs, pool, landscaping, driveways, boundary walls and other external works.
- External-works totals must separately cover the likely pool, landscaping, driveways, boundaries, services and site works for this subject plot.
        ...
        input=[
            {
                "role": "system",
                "content": (
                    "You are a conservative Cyprus development valuer. Use web search, distinguish ordinary from luxury micro-markets, "
                    "and return only the requested structured result. Never make land, construction and completed-sale figures internally inconsistent."
                ),
            },
...
def build_viable_development_options(
    ...
    def economics_for(
        ...
        if property_type == "house" and count_h <= 1.0:
            ...
            if direct_low is not None and direct_high is not None:
                gdv_low = max(gdv_low, min(direct_low, direct_high))
                gdv_high = max(gdv_high, max(direct_low, direct_high))
                sale_valuation_basis = "subject_site_detached_house_total_value_primary"
    ...
    for card in cards:
        ...
        residual_candidates.append({"low": max(0.0, r_low), "high": max(0.0, r_high), "option_id": card.get("id")})
    best_residual = max(
        residual_candidates,
    ...
    if parcel_area and land_low_m2 is not None and land_high_m2 is not None:
        asking_discount = _num(market_root.get("land_asking_to_achievable_discount_percent"))
        if asking_discount is None:
            asking_discount = LAND_ASKING_DISCOUNT_PERCENT
        asking_discount = min(max(asking_discount, 0.0), 30.0)

        # Larger plots usually trade below the simple small-plot €/m² headline.
        if parcel_area >= 4000:
            size_discount = 0.15
        elif parcel_area >= 2500:
            size_discount = 0.12
        elif parcel_area >= 1500:
            size_discount = 0.09
        elif parcel_area >= 800:
            size_discount = 0.05
        else:
            size_discount = 0.02
        adjustment = (1.0 - asking_discount / 100.0) * (1.0 - size_discount)
        direct_low = parcel_area * min(land_low_m2, land_high_m2) * adjustment
        direct_high = parcel_area * max(land_low_m2, land_high_m2) * adjustment

        residual_cap_applied = False
        if best_residual and best_residual["high"] >= direct_low * 0.65:
            # Residual value is a ceiling check, not a replacement for comparable
            # land evidence. Apply it only when it is reasonably close to the
            # comparable-land result; otherwise flag the feasibility conflict
            # without collapsing the market estimate to a distressed figure.
            direct_high = min(direct_high, best_residual["high"] * LAND_RESIDUAL_CAP_MULTIPLIER)
            residual_cap_applied = True
            if direct_high < direct_low:
                direct_low = direct_high * 0.90
        value_estimate = {
            "low_eur": round(max(direct_low, 0.0)),
            "high_eur": round(max(direct_high, direct_low, 0.0)),
            "method": "adjusted_land_asking_with_residual_cap" if residual_cap_applied else "adjusted_land_asking",
            "residual_cap_applied": residual_cap_applied,
            "asking_discount_percent": round(asking_discount, 1),
            "large_plot_discount_percent": round(size_discount * 100.0, 1),
            "raw_land_asking_eur_m2": {"low": round(min(land_low_m2, land_high_m2)), "high": round(max(land_low_m2, land_high_m2))},
        ...
        value_estimate = {
            ...
            "method": "best_use_development_residual",
            "best_use_residual_context": best_residual,
        }
