...
APP_BUILD = "2026-07-16-online-first-v3"
...
MARKET_ANALYSIS_CACHE_TTL_SECONDS = max(900, int(os.getenv("PLANA_MARKET_RESEARCH_CACHE_TTL_SECONDS", "21600")))
...
async def parcel_details(parcel_id: int = Query(gt=0)) -> dict[str, Any]:
    ...
    fallback_market = _fallback_market_analysis(details)
...
async def build_hybrid_market_analysis(
    ...
    cache_key = json.dumps({
        "district": parcel.get("district"), "municipality": parcel.get("municipality"), "quarter": parcel.get("quarter"),
        "zone": [item.get("zone") for item in parcel_details.get("planning_zones") or []],
        "size_band": round((_usable_number(parcel.get("parcel_extent_m2")) or 0) / 250) * 250,
        "lat": round(centroid_lat, 3) if centroid_lat is not None else None,
        "lon": round(centroid_lon, 3) if centroid_lon is not None else None,
        "hybrid": 1,
    ...
    if LIVE_MARKET_ENABLED:
        try:
            live = await asyncio.wait_for(asyncio.to_thread(_run_live_market_research, parcel_details), timeout=MARKET_RESEARCH_TIMEOUT_SECONDS)
            merged = _apply_live_research(merged, live)
        except Exception as exc:
            print(f"PLANA targeted market research fallback: {exc!r}", file=sys.stderr)
...
def build_viable_development_options(
    ...
    market_root = opportunity.get("market_analysis") or _fallback_market_analysis(parcel_details)
...
function renderNearby(){const amenities=locationInfo?.amenities||[],market=opportunity?.market_analysis||{},property=market.property_analyses||{};const marketCards=[["Apartments",property.apartment],["Houses",property.house],["Offices",property.office],["Shops",property.shop],["Land",property.land]].filter(([,value])=>value);$("nearby").innerHTML=`
<div class="card"><h2>Amenities</h2>${amenities.length?`<div class="amenity-grid">${amenities.map(amenityButton).join("")}</div>`:`<div class="empty-result">Nearby places are being refreshed.</div>`}</div>
