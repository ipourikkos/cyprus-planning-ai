...
<style>
:root{--ink:#14231a;--muted:#6f7a73;--line:#e3e8e4;--bg:#f5f7f5;--card:#fff;--green:#173f2b;--soft:#edf3ef;--warn:#fff8e8;--danger:#a33}
*{box-sizing:border-box}body{margin:0;font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:var(--ink);background:var(--bg)}button,input{font:inherit}
.app{display:grid;grid-template-columns:minmax(0,1.35fr) minmax(390px,.65fr);height:100vh}.map-shell{position:relative;min-width:0}#map{height:100%;width:100%}
.brand{position:absolute;z-index:900;left:18px;top:18px;background:rgba(255,255,255,.95);border:1px solid var(--line);border-radius:16px;padding:12px 15px;box-shadow:0 10px 32px rgba(20,35,26,.10)}.brand b{font-size:18px}.brand span{display:block;font-size:11px;color:var(--muted);margin-top:2px}
.search{position:absolute;z-index:900;left:18px;top:88px;width:min(420px,calc(100% - 36px))}.search form{display:flex;background:#fff;border:1px solid var(--line);border-radius:14px;padding:6px;box-shadow:0 10px 30px rgba(20,35,26,.10)}.search input{flex:1;border:0;outline:0;padding:10px 12px}.search button{border:0;background:var(--green);color:white;border-radius:10px;padding:0 16px;font-weight:750}.result{display:block;width:100%;text-align:left;background:#fff;border:1px solid var(--line);padding:10px 12px;margin-top:6px;border-radius:10px;cursor:pointer}
.panel{overflow:auto;border-left:1px solid var(--line);background:#fafbfa;padding:22px}.empty{min-height:70vh;display:grid;place-items:center;text-align:center;color:var(--muted);padding:50px}.empty strong{display:block;color:var(--ink);font-size:26px;margin-bottom:8px}.topline{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:18px}.eyebrow{font-size:11px;letter-spacing:.12em;text-transform:uppercase;color:var(--muted);font-weight:800}.parcel-title{font-size:25px;font-weight:850;margin-top:4px}.sub{font-size:13px;color:var(--muted);margin-top:4px}.pill{font-size:11px;font-weight:800;background:var(--soft);border:1px solid #d9e5dc;padding:6px 9px;border-radius:999px;white-space:nowrap}
.section{background:var(--card);border:1px solid var(--line);border-radius:18px;padding:16px;margin-bottom:12px}.section h2{font-size:15px;margin:0 0 12px}.hero-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:8px}.metric{background:var(--bg);border-radius:12px;padding:11px}.metric .k{font-size:11px;color:var(--muted);margin-bottom:4px}.metric .v{font-size:20px;font-weight:850}.metric .u{font-size:12px;font-weight:600;color:var(--muted)}
.option-tabs{display:flex;gap:7px;overflow:auto;margin-bottom:12px}.tab{border:1px solid var(--line);background:#fff;border-radius:999px;padding:8px 12px;font-size:12px;font-weight:750;cursor:pointer;white-space:nowrap}.tab.active{background:var(--green);color:#fff;border-color:var(--green)}.option-hero{font-size:27px;font-weight:900;margin:3px 0 10px}.option-metrics{display:grid;grid-template-columns:repeat(3,1fr);gap:7px}.mini{border:1px solid var(--line);border-radius:11px;padding:9px}.mini .k{font-size:10px;color:var(--muted)}.mini .v{font-size:14px;font-weight:800;margin-top:4px}.strategy{margin-top:12px;padding-top:11px;border-top:1px solid var(--line);display:grid;gap:7px}.strategy div{font-size:13px;line-height:1.4}.strategy div:before{content:"✓";color:#2b7a4b;font-weight:900;margin-right:7px}
.value{font-size:29px;font-weight:900}.value-note{font-size:12px;color:var(--muted);margin-top:5px}.rangebar{height:7px;border-radius:999px;background:#e7ece8;margin-top:12px;overflow:hidden}.rangebar span{display:block;height:100%;width:72%;background:var(--green);border-radius:999px}
.issue{padding:10px 11px;border-radius:11px;background:var(--warn);font-size:13px;margin-top:7px}.details summary{cursor:pointer;font-weight:750;font-size:13px}.details-body{font-size:12px;color:var(--muted);line-height:1.55;padding-top:10px}.source{border-top:1px solid var(--line);padding:8px 0}.hidden{display:none!important}.loading{color:var(--muted);font-size:13px;padding:8px 0}
.ai{position:sticky;bottom:0;background:linear-gradient(to top,#fafbfa 80%,rgba(250,251,250,0));padding-top:24px}.ai-box{background:#fff;border:1px solid #cfd8d1;border-radius:16px;padding:9px;box-shadow:0 10px 30px rgba(20,35,26,.08)}.ai-row{display:flex;gap:8px}.ai input{flex:1;border:0;outline:0;padding:9px}.ai button{border:0;background:var(--green);color:#fff;border-radius:10px;padding:9px 13px;font-weight:800}.ai-answer{font-size:13px;line-height:1.55;padding:10px 8px 3px;white-space:pre-wrap}.ai-label{font-size:10px;color:var(--muted);font-weight:800;padding:0 8px 5px}
@media(max-width:900px){.app{grid-template-columns:1fr;grid-template-rows:48vh auto;height:auto;min-height:100vh}.panel{border-left:0;border-top:1px solid var(--line);overflow:visible}.map-shell{height:48vh}.option-metrics{grid-template-columns:1fr 1fr}.ai{position:relative}}
</style></head><body>
<div class="app"><div class="map-shell"><div id="map"></div><div class="brand"><b>PLANA.CY</b><span>Parcel intelligence</span></div><div class="search"><form id="searchForm"><input id="searchInput" placeholder="Search address or place in Cyprus"><button>Search</button></form><div id="results"></div></div></div>
<aside class="panel"><div id="empty" class="empty"><div><strong>Click a parcel</strong>PLANA will calculate planning capacity, viable development options and market value.</div></div><div id="content" class="hidden">
<div class="topline"><div><div class="eyebrow">Selected parcel</div><div id="parcelTitle" class="parcel-title">—</div><div id="parcelSub" class="sub">—</div></div><span id="confidence" class="pill">Analysing</span></div>
<section class="section"><div id="headlineMetrics" class="hero-grid"></div></section>
<section class="section"><h2>Development options</h2><div id="optionTabs" class="option-tabs"></div><div id="optionBody" class="loading">Calculating viable options…</div></section>
<section class="section"><h2>Indicative value</h2><div id="valueBody" class="loading">Checking market evidence…</div></section>
<section id="issuesSection" class="section hidden"><h2>Things that matter</h2><div id="issues"></div></section>
<section class="section details"><details><summary>How PLANA calculated this</summary><div id="thought" class="details-body">Planning intelligence runs in the background and will appear here only when expanded.</div></details></section>
<div class="ai"><div class="ai-box"><div class="ai-label">ASK PLANA ABOUT THIS PARCEL</div><div class="ai-row"><input id="aiQuestion" placeholder="e.g. Can I put the parking in a basement?"><button id="askAiBtn" type="button">Ask</button></div><div id="aiAnswer" class="ai-answer hidden"></div></div></div>
</div></aside></div>
...
const fmt=(v,d=0)=>v==null||v===""?"—":Number(v).toLocaleString(undefined,{maximumFractionDigits:d}); const money=v=>v==null?"—":"€"+Number(v).toLocaleString(undefined,{maximumFractionDigits:0});
let currentSite=null,currentPlanning=null,currentOpportunity=null,currentProposals=null,selectionRequestId=0,parcelLayer=null,activeOption=0;
const map=L.map("map",{zoomControl:false}).setView([35.05,33.2],9);
L.control.zoom({position:"bottomleft"}).addTo(map);
L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{
  maxZoom:20,
  attribution:"&copy; OpenStreetMap contributors"
}).addTo(map);
if(L.esri&&L.esri.dynamicMapLayer){
  L.esri.dynamicMapLayer({
    url:"https://eservices.dls.moi.gov.cy/arcgis/rest/services/National/CadastralMap_EN/MapServer",
    opacity:0.72,
    layers:[0]
  }).addTo(map);
}
...
function showParcel(){const p=currentSite.parcel||{},z=(currentSite.planning_zones||[]).map(x=>x.zone).filter(Boolean).join(" / ");$("empty").classList.add("hidden");$("content").classList.remove("hidden");$("parcelTitle").textContent=`Parcel ${p.parcel_number||p.parcel_id||"—"}`;$("parcelSub").textContent=[p.quarter,p.municipality,p.district].filter(Boolean).join(" · ");const pot=currentSite.development_potential||{};$("headlineMetrics").innerHTML=metric("Area",fmt(p.parcel_extent_m2,0),"m²")+metric("Planning zone",z||"—")+metric("Buildable floor area",fmt(pot.theoretical_max_floor_area_m2,0),"m²")+metric("Ground coverage",fmt(pot.theoretical_max_ground_coverage_m2,0),"m²");}
function renderOptions(){const opts=currentProposals?.development_options||[];if(!opts.length){$("optionTabs").innerHTML="";$("optionBody").innerHTML='<div class="loading">No supported development option could be established from the current rules and parcel data.</div>';return}activeOption=Math.min(activeOption,opts.length-1);$("optionTabs").innerHTML=opts.map((x,i)=>`<button class="tab ${i===activeOption?'active':''}" data-i="${i}">${esc(x.label)}</button>`).join("");document.querySelectorAll('.tab').forEach(b=>b.onclick=()=>{activeOption=Number(b.dataset.i);renderOptions()});const x=opts[activeOption];const metrics=(x.metrics||[]).filter(m=>m.value!=null).map(m=>`<div class="mini"><div class="k">${esc(m.label)}</div><div class="v">${esc(m.value)} ${esc(m.unit||"")}</div></div>`).join("");$("optionBody").innerHTML=`<div class="eyebrow">${esc(x.status==='allowed'?'Supported by retrieved planning sources':'Feasibility scenario')}</div><div class="option-hero">${esc(x.headline)}</div><div class="option-metrics">${metrics}</div>${x.maximisation?.length?`<div class="strategy">${x.maximisation.map(s=>`<div>${esc(s)}</div>`).join("")}</div>`:""}${x.reason?`<div class="value-note">${esc(x.reason)}</div>`:""}`;}
function renderValue(){const v=currentProposals?.value_estimate,p=currentSite?.parcel||{},m=currentOpportunity?.market_analysis||{};if(v){$("valueBody").innerHTML=`<div class="value">${money(v.low_eur)}–${money(v.high_eur)}</div><div class="value-note">${esc(v.basis)}</div><div class="rangebar"><span></span></div>${m.relevant_observation_count?`<div class="value-note">Based on ${esc(m.relevant_observation_count)} relevant asking-market observations · ${esc(m.confidence||'low')} confidence</div>`:""}`;}else if(p.price_2021){$("valueBody").innerHTML=`<div class="value">${money(p.price_2021)}</div><div class="value-note">DLS 2021 General Valuation context only. PLANA does not yet have enough market/cost evidence to estimate current plot or built-structure value.</div>`;}else{$("valueBody").innerHTML='<div class="loading">Not enough market evidence to estimate value.</div>';}}
function renderIssues(){const issues=[];const p=currentSite?.parcel||{},sc=currentSite?.spatial_checks||{};if(p.is_preserved)issues.push("Preserved-property flag.");if(p.is_ancient)issues.push("Ancient-property flag.");if(sc.coast_protection)issues.push("Coast-protection overlap.");if(sc.state_land)issues.push("State-land overlap.");(currentPlanning?.capacity_caveats||[]).slice(0,3).forEach(x=>issues.push(x));const s=currentProposals?.setbacks;if(s?.status==='working_assumption')issues.push(s.note);$("issuesSection").classList.toggle("hidden",!issues.length);$("issues").innerHTML=issues.map(x=>`<div class="issue">${esc(x)}</div>`).join("");}
function renderThought(){const a=currentPlanning||{},rules=currentSite?.structured_rule_analysis||{},s=currentProposals?.setbacks||{};const provisions=(a.material_provisions||[]).map(x=>`<div class="source"><b>${esc(x.title)}</b><br>${esc(x.finding)}${x.development_impact?`<br><span>${esc(x.development_impact)}</span>`:""}</div>`).join("");$("thought").innerHTML=`<b>Working envelope</b><br>Front ${esc(s.front_m)} m · side ${esc(s.side_m)} m · rear ${esc(s.rear_m)} m${s.envelope_width_m?` · approx. ${esc(s.envelope_width_m)} × ${esc(s.envelope_length_m)} m envelope`:''}.<br><br><b>Rule engine</b><br>${esc(rules.source_precedence||'Order 4/2026 structured checks active.')} ${esc(a.summary||'')}${provisions?`<br>${provisions}`:''}`;}
async function post(url,body){const r=await fetch(url,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});const d=await r.json();if(!r.ok)throw new Error(d.detail||"Request failed");return d}
async function analyse(requestId,parcelId){
  const geom=currentSite.geometry_metrics||{};
  // Critical path: deterministic proposals first. No AI, market or extra GIS wait.
  try{
    ...
    $("confidence").textContent="Preliminary";
    renderOptions();renderIssues();renderThought();
  }catch(e){if(requestId===selectionRequestId)$("optionBody").innerHTML=`<div class="loading">Could not calculate options: ${esc(e.message||e)}</div>`;}

  // Market/value enriches independently.
  post('/api/parcel-opportunity-analysis',{parcel_id:parcelId,assumptions:{},centroid_lat:geom.centroid_lat??null,centroid_lon:geom.centroid_lon??null}).then(async opp=>{
    if(requestId!==selectionRequestId)return;
    currentOpportunity=opp;
    currentProposals=(currentPlanning?await post('/api/parcel-proposals',{parcel_id:parcelId,planning_analysis:currentPlanning,opportunity_analysis:currentOpportunity}):opp.proposals)||currentProposals;
    if(requestId!==selectionRequestId)return;
    renderOptions();renderValue();renderThought();
  }).catch(()=>{if(requestId===selectionRequestId)renderValue();});

  // Only critical parcel constraints run automatically; they never block options/value.
  post('/api/site-extra',{parcel_feature:currentSite.parcel_feature}).then(extra=>{
    if(requestId!==selectionRequestId)return;
    currentSite={...currentSite,...extra};renderIssues();
  }).catch(()=>{});

  // Hidden AI is deliberately last and background-only. It refines use/setback evidence.
  const startPlanning=()=>post('/api/parcel-planning-analysis',{parcel_id:parcelId}).then(async planning=>{
    if(requestId!==selectionRequestId)return;
    currentPlanning=planning;
    currentProposals=await post('/api/parcel-proposals',{parcel_id:parcelId,planning_analysis:currentPlanning,opportunity_analysis:currentOpportunity});
    if(requestId!==selectionRequestId)return;
    $("confidence").textContent=currentPlanning?.confidence?`${currentPlanning.confidence} confidence`:"Preliminary";
    renderOptions();renderIssues();renderThought();
  }).catch(()=>{});
  if('requestIdleCallback' in window)requestIdleCallback(startPlanning,{timeout:1200});else setTimeout(startPlanning,350);
}
async function selectSite(lat,lon){const requestId=++selectionRequestId;currentPlanning=currentOpportunity=currentProposals=null;activeOption=0;try{const br=await fetch(`/api/parcel-basic?lat=${encodeURIComponent(lat)}&lon=${encodeURIComponent(lon)}`),basic=await br.json();if(!br.ok)throw new Error(basic.detail||'Parcel lookup failed');if(requestId!==selectionRequestId)return;if(parcelLayer)map.removeLayer(parcelLayer);parcelLayer=L.geoJSON(basic.parcel_feature,{style:{color:'#173f2b',weight:3,fillOpacity:.16}}).addTo(map);map.fitBounds(parcelLayer.getBounds(),{padding:[35,35]});$("empty").classList.add("hidden");$("content").classList.remove("hidden");$("parcelTitle").textContent=`Parcel ${basic.parcel_number||basic.parcel_id||"—"}`;$("parcelSub").textContent="Loading official parcel details…";$("headlineMetrics").innerHTML=metric("Area","…")+metric("Planning zone","…")+metric("Buildable floor area","…")+metric("Ground coverage","…");$("optionBody").innerHTML='<div class="loading">Calculating capacity…</div>';$("valueBody").innerHTML='<div class="loading">Checking market evidence…</div>';const dr=await fetch(`/api/parcel-details?parcel_id=${encodeURIComponent(basic.parcel_id)}`),details=await dr.json();if(!dr.ok)throw new Error(details.detail||'Parcel details failed');if(requestId!==selectionRequestId)return;currentSite={...details,parcel_feature:basic.parcel_feature,geometry_metrics:basic.geometry_metrics||{}};showParcel();analyse(requestId,basic.parcel_id);}catch(e){if(requestId===selectionRequestId)alert(e.message||e)}}
